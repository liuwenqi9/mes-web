...
import com.egdfrm.unit.service.productionManagement.IPartsPackingService;
 ...
 */
@Controller
@RequestMapping("parts/packing")
public class PartsPackingController {

    @Autowired
    private IPartsPackingService iPartsPackingService;
    ...
    @RequestMapping("init")
    public String init(){
        return "unit/productionManagement/parts_packing";
    }
    ...
    public String queryAll(String workOrderNumber){
        List<Map<String,Object>> list = iPartsPackingService.queryAll(workOrderNumber);
    ...
    public String queryPackingAll(String workOrderNumber){
        List<Map<String,Object>> list = iPartsPackingService.queryPackingAll(workOrderNumber);
        return JSON.toJSONString(list);
    }
    ...
    public String savePackingNumber(@CurrentLoginInfo UserAuthenRealm.ShiroUser user,String wipEntityIds, String inventoryItemIds, String pacgingNums){
        ...
        try {
            ...
            int count = iPartsPackingService.savePackingNumber(user.getUserId(),strSipEntityIds,strInventoryItemIds,strPacgingNums);
            if(count > 0){
    ...
    public String printerPacking(HttpServletRequest request,String packingBarcodeIds){
        ...
        try {
            ...
            for (int i = 0; i < barcodeIds.length; i++) {
                 ...
                 * 头信息查询
                 */
                Map<String,Object> map = iPartsPackingService.findPackingById(barcodeId);
                /************** 第一张图片 ****************/
                 ...
                 * 行信息查询
                 */
                List<Map<String,Object>> listRows = iPartsPackingService.getRows(barcodeId);
                //数据集总条数
                ...
                for (int j = 0; j < pages; j++) {
                    ...
                    CreatePdf.pdfAddTableDataByParts(document,map,head,fs,rows);
                    //添加页脚
            ...
            document.close();
            //修改打印状态
            int result = iPartsPackingService.updateStatus(barcodeIds);
            if(result > 0){
                json.setSuccess(true);
                //访问PDF地址
