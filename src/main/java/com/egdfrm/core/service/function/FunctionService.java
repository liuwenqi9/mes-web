 ...
 * Copyright (C) egdfrm.
 */
package com.egdfrm.core.service.function;
...
public class FunctionService extends BaseService {
            ...
            for (Map<String, Object> menu : menuList) {
                ...
                if (!CollectionUtils.isEmpty(functionList)) {
                    for (Map<String, Object> function : functionList) {
                        ...
                        script.append(" <a href=\"javascript:void(0);\" onclick=\"addTab('" + functionCode + "','" + functionName + "','"
     ...
     * @return string
     */
    public String loadMenu(String loginName) {
    	StringBuffer script = new StringBuffer();
    	List<Map<String, Object>> listAppData = this.loadAppData(loginName);
    	for (int i = 0; i < listAppData.size(); i++) {
    		String appCode1 = listAppData.get(i).get("FUNCTION_CODE").toString();
    		String menuName1 = listAppData.get(i).get("FUNCTION_NAME").toString();
    		script.append(" <li class=\"treeview\"> <a href=\"javascript:void(0);\"> <i class=\"fa fa-share\"></i> <span>"
    				+ menuName1+ " </span> <i class=\"fa fa-angle-left pull-right\"></i> </a> <ul class=\"treeview-menu\"> ");
            //获得menu
            List<Map<String, Object>> menuList = null;
            menuList = this.getFunctionListMenu(loginName, appCode1);
            if (!CollectionUtils.isEmpty(menuList)) {
                for (Map<String, Object> menu : menuList) {
                    String menuName2 = null;
                    String menuCode2 = null;
                    menuName2 = menu.get("FUNCTION_NAME").toString();
                    menuCode2 = menu.get("FUNCTION_CODE").toString();
                    script.append(" <li> <a href=\"javascript:void(0);\"><i class=\"fa fa-circle-o\"></i> "
                    		+ menuName2 + " <i class=\"fa fa-angle-left pull-right\"></i></a> <ul class=\"treeview-menu\"> ");
                    //获得function
                    List<Map<String, Object>> functionList = null;
                    functionList = this.getFunctionList(loginName, menuCode2);
                    if (!CollectionUtils.isEmpty(functionList)) {
                        for (Map<String, Object> function : functionList) {
                            String functionCode = null;
                            String functionName = null;
                            String reqeustUrl = null;
                            //地址为空则不再显示在菜单栏 modify by sjf 20170103
                            if( function.get("REQUEST_URL") != null && !StringUtils.isEmpty(function.get("REQUEST_URL").toString())){
	                            functionCode = function.get("FUNCTION_CODE").toString();
	                            functionName = function.get("FUNCTION_NAME").toString();
	                            reqeustUrl = function.get("REQUEST_URL").toString();
	                            script.append(" <li><a href=\"javascript:void(0);\" onclick=\"toFunction('" + functionCode + "','" + functionName + "','"
	                                + reqeustUrl + "');\"><i class=\"fa fa-circle-o\"></i> "
	                            		+ functionName + " </a></li> ");
                            }
                        }
                    }
            ...
            for (Map<String, Object> appNodeItem : appNode) {
                ...
                for (Map<String, Object> menuNodeItem : menuNode) {
                    ...
                    for (Map<String, Object> functionNodeItem : functionNode) {
                        ...
                        rvFunctionItem.put("text", functionName);
                        //获得权限明细
                        if (appendPurviewChild) {
                            ...
                            for (Map<String, Object> functionPurviewItem : functionPurviewList) {
                                ...
                                if (purview != null && functionCode.equals(purviewfunctionCode)) {
                                    ...
                                    rvPurviewItem.put("text", purview.getPurviewName()+"("+purview.getPurviewCode()+")");
                                    rvPurviewList.add(rvPurviewItem);
                                }
                            }
                            rvFunctionItem.put("nodes", rvPurviewList);
                        }
                        rvFunctionList.add(rvFunctionItem);
                    }
                    rvMenuItem.put("nodes", rvFunctionList);
                    rvMenuList.add(rvMenuItem);
                }
                rvAppItem.put("nodes", rvMenuList);
    ...
    public List<Map<String, Object>> loadFunctionPurviewTreeNode(List<Map<String, Object>> functionPurviewList,
            boolean appendPurviewChild) {
        if (!CollectionUtils.isEmpty(functionPurviewList)) {
    ...
    public List<Map<String, Object>> getAppNode(boolean hasChildren) {
        ...
        APP_SQL.append(" SELECT * FROM TT_FUNCTION t");
        ...
        if (hasChildren) {
            if (!CollectionUtils.isEmpty(appNode)) {
                for (int i = 0; i < appNode.size(); i++) {
                	appNode.get(i).put("text", appNode.get(i).get("FUNCTION_NAME"));
                    ...
                    if (!CollectionUtils.isEmpty(accordionNode)) {
                        appNode.get(i).put("nodes", accordionNode);
                    }
                }
            }
        }
    ...
    public List<Map<String, Object>> getAccordionNode(String parentFunctionCode, boolean hasChildren) {
        ...
        ACCORDION_NODE_SQL.append(" SELECT * FROM TT_FUNCTION t");
            ...
            if (!CollectionUtils.isEmpty(accordionNode)) {
                if (!CollectionUtils.isEmpty(accordionNode)) {
                    for (int i = 0; i < accordionNode.size(); i++) {
                        ...
                        accordionNode.get(i).put("text", item.get("FUNCTION_NAME"));
                        ...
                        if (!CollectionUtils.isEmpty(functionNode)) {
                            accordionNode.get(i).put("nodes", functionNode);
                        }
                    }
                }
            }
    ...
    public List<Map<String, Object>> getFunctionNode(String parentFunctionCode, boolean hasChildren) {
        ...
        FUNCTION_NODE_SQL.append(" SELECT * FROM TT_FUNCTION t");
        ...
        if (!CollectionUtils.isEmpty(functionNode)) {
            for (int i = 0; i < functionNode.size(); i++) {
            	functionNode.get(i).put("text", functionNode.get(i).get("FUNCTION_NAME"));
            }
        }
        return functionNode;
    }
