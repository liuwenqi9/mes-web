...
public class CSVFileUtil {
    ...
    public static void main(String[] arge) {
        try {
            CSVFileUtil c = new CSVFileUtil("F:\\TI_RECALLED_ACTIVITY.csv", "GBK");
            int row = 1;
            while (true) {
                ...
                ArrayList lineArray = c.fromCSVLinetoArray(line);
                if (lineArray != null) {
                    for (int i = 0; i < lineArray.size(); i++) {
                        String item = (String) lineArray.get(i);
                        System.out.println("item[" + row + "][" + i + "]=" + item);
                    }
                }
                row++;
            }
