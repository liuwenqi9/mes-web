...
$(function() {
	// 新增按钮
	$('#supplier_edit_search_btn').click(function() {
		// 弹出新增窗口
		$('#supplier_search_window_add').window({
			title : '选择供应商',
			collapsible : false,
			minimizable : false,
			maximizable : false,
			...
			width : 570,
			height : 450,
			href : basePath + 'userController/toSupplierEdit'
		});
	});
	...
	$('#user_form_edit').form({
		onLoadSuccess : function(data) { // 加载完毕后在格式化表单
			...
			$('#userStatus_edit').combobox({
				required : true,
				method : 'get',
				url : basePath + 'static/egdfrm/json/enableOrDisable.json',
				valueField : 'value',
				textField : 'text',
				editable : false
			});
