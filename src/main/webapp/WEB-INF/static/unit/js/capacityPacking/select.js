...
$(function() {
	...
	//	fit : true,
		...
		onDblClickRow:function(rowIndex, rowData) { //alert(rowData.INVENTORY_ITEM_ID);
			$('#materialCode_add').val(rowData.SEGMENT1);
			$('#materialDes_add').val(rowData.DESCRIPTION);
			$('#packingType_add').val(rowData.PROD_TYPE);
			$('#organization_id_add').val(rowData.ORGANIZATION_ID);
			$('#inventory_item_id_add').val(rowData.INVENTORY_ITEM_ID);
			//INVENTORY_ITEM_ID,ORGANIZATION_ID
			$('#table_bb3').window('close');
